...
// 异步测试
describe('Asynchronous Test 异步测试套件', function () {
  var orderList = []
  it('Case 1 "done()" 使用 done() 异步获取数据并通知', function (done) {
    fzsOrder.getAsynData(done)
  })

  it('Case 2 根据 case1 异步获取之后，查看数据是否正确获取', function () {
    orderList = fzsOrder.data
    expect(orderList.length > 0).to.be.equal(true)
  })
})
